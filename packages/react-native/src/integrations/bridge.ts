...
export const handleBridge = async ({
  ...
  const handleThrow = () => {
    webview.injectJavaScript(SAFE_NATIVE_EMITTER_THROW(bridgeId, `${method}-${eventId}`));
  };
  ...
  try {
    ...
    webview.injectJavaScript(
      SAFE_NATIVE_EMITTER_EMIT(bridgeId, `${method}-${eventId}`, response),
    );
  } catch (error) {
    handleThrow();
    console.error(error);
...
export const SAFE_NATIVE_EMITTER_EMIT = (bridgeId: string, eventName: string, data: unknown) => {
...
export const SAFE_NATIVE_EMITTER_THROW = (bridgeId: string, eventName: string) => `
if (window.nativeEmitter['${bridgeId}']) {
